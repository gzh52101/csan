...
export default {
  ...
  computed: {
    ...
    gettotal() {
      return this.$store.state.Joachern.total || 0;
    },
  },
  methods: {
    handleadd() {
    ...
    searchitem() {
      this.JavaInfo = this.JavaInfo.filter((item) => {
    ...
    getJoachernData() {
      ...
      let payload = {
        ...
        data: { dbteb: "dataJoachern", init: init, end: size },
        type: "post",
      };
      this.$store.dispatch("ajaxFrom", payload).then(
        (res) => {
          let result = JSON.parse(res);
          this.$store.state.Joachern.JoachernInfo = result.data;
        },
        (err) => {
          console.log(err);
        }
    ...
    getDatatotal() {
      ...
      this.$store.dispatch("ajaxFrom", payload).then(
        (res) => {
          let result = JSON.parse(res);
          this.$store.state.Joachern.total = result.data.length;
        },
        (err) => {
          console.log(err);
        }
