    ...
    <div id="bigBox" v-show="showNav">
      <div class="title" v-show="isShow">
        ...
        <a
          v-if="isLogin"
          @click="goto('/mine')"
          style="margin-top: 6px; margin-right: 45px; font-size: 16px"
          >用户:{{ userInfo.username }}</a
        >
      ...
      <div class="main" v-show="isSeach">
        <router-link
          ...
          @click="addButtonActive(item.name)"
        >
          {{ item.text }}</router-link
        >
      ...
      <div class="da-fab" v-if="fabu">
        <button @click="FBu">发布</button>
      </div>
    </div>
